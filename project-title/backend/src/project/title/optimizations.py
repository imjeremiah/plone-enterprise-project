...
Provides cached and optimized functions for expensive operations,
particularly dashboard data aggregation and real-time updates.
Implements 30-second caching for dashboard queries as specified in Phase 4.
"""
...
from ..catalog import get_performance_catalog
...
def _cache_key_user_context(method, self, *args):
    ...
    return (time.time() // 30, user.getId() if user else 'anonymous', portal.absolute_url())


@ram.cache(_cache_key_time_30s)
def get_dashboard_aggregates():
    """
    Cache expensive dashboard queries for 30 seconds
    ...
    perf_catalog = get_performance_catalog()
    ...
    try:
        ...
        try:
            ...
            # Count active timers
            active_timers = sum(1 for timer in timer_data.values() 
                              if timer.get('is_running', False))
            ...
            if participation_data:
                ...
                if today_picks:
                    ...
                    if pick_counts:
                        ...
                        fairness = max(0, 100 - (variance * 10))  # Scale variance to percentage
                        aggregates['participation']['fairness_score'] = round(fairness, 1)
                        aggregates['participation']['students_picked_today'] = len(today_picks)
        ...
        api.portal.show_message(f"Dashboard aggregation warning: {str(e)}", request=getattr(portal, 'REQUEST', None))
