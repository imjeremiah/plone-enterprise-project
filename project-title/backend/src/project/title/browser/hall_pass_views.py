...
class HallPassManagerView(BrowserView):
    ...
    def issue_pass(self):
        ...
        try:
            ...
            self.request.response.setHeader("Content-Type", "application/json")
            return json.dumps({"error": "Failed to issue pass", "details": str(e)})
    ...
    def get_passes_data(self):
        ...
        try:
            ...
            return json.dumps(
                {
                    ...
                    "statistics": {
                        ...
                        "avg_duration": sum(
                            p["duration_minutes"] for p in all_passes.values()
                        )
                        / max(total_passes, 1),
                    },
                    "alerts": alerts,
            ...
            self.request.response.setStatus(500)
            self.request.response.setHeader("Content-Type", "application/json")
            return json.dumps({"error": "Failed to load data", "details": str(e)})
    ...
    def issue_pass_with_workflow(self):
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.warning(
                f"Workflow enhancement failed, basic functionality preserved: {e}"
            )
        ...
        try:
            ...
            if pass_data:
                ...
                try:
                    ...
                    notify(event)
                    logger.info(
                        f"✅ Hall pass return event fired after {duration} minutes"
                    )
                except Exception as event_error:
                    logger.info(
                        f"Return event firing skipped (non-critical): {event_error}"
                    )
