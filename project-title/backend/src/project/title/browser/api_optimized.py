            ...
            for brain in brains:
                try:
                    ...
                    if hasattr(brain, "hall_pass_issue_time"):
                        ...
                        pass_data["issue_time"] = annotations.get(
                            "hall_pass_issue_time"
                        )
                        pass_data["duration"] = annotations.get(
                            "hall_pass_duration", 10
                        )
                        pass_data["student"] = annotations.get(
                            "student_name", "Unknown"
                        )
                        pass_data["destination"] = annotations.get(
...
class OptimizedSeatingChartAPI(BrowserView):
    ...
    @ram.cache(
        lambda *args: (time.time() // 30, args[1] if len(args) > 1 else "default")
    )
    def get_chart_optimized(self, chart_id):
        ...
        try:
            ...
            chart_data = {
                ...
                "last_modified": arrangement.get(
                    "last_modified", chart_obj.modified().ISO8601()
                ),
    ...
    def list_charts_optimized(self):
        ...
        try:
            ...
            self.request.response.setHeader("Content-Type", "application/json")
            return json.dumps(
                {"charts": charts, "count": len(charts), "total_available": len(brains)}
            )
