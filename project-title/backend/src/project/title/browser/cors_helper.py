...
import logging
from ..security import validate_cors_origin, get_security_headers, PRODUCTION_SECURITY_CONFIG
...
def set_cors_headers(request, response):
    ...
    if origin:
        ...
        allowed_origins = PRODUCTION_SECURITY_CONFIG.get('cors_origins', DEVELOPMENT_ORIGINS)
    ...
    def decorator(view_method):
        def wrapper(self):
            ...
            if rate_limit_action:
                ...
                if user:
                    user_id = user.getId()
                    if not check_rate_limit(user_id, rate_limit_action, rate_limit_per_hour):
