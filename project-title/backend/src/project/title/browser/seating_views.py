    ...
    def __call__(self):
        ...
        try:
            ...
            if hasattr(self.context, "update_position"):
                ...
                self.request.response.setStatus(400)
                return json.dumps(
                    {"error": "Context does not support position updates"}
                )
        ...
        try:
            ...
            try:
                if hasattr(self.context, "students") and self.context.students:
                    len(self.context.students)

                # fire_seating_chart_updated(self.context, student_count=student_count)
            except Exception as event_error:
                logger.info(
                    f"Seating event firing skipped (non-critical): {event_error}"
                )
    ...
    def get_seating_statistics(self):
        ...
        try:
            ...
            stats = {
                ...
                "available_positions": (self.context.grid_rows * self.context.grid_cols)
                - len(students),
                "utilization_percent": round(
                    (len(students) / (self.context.grid_rows * self.context.grid_cols))
                    * 100,
                    1,
                ),
...
class SeatingChartStatsView(BrowserView):
    ...
    def __call__(self):
        ...
        try:
            ...
            total_desks = getattr(self.context, "grid_rows", 5) * getattr(
                self.context, "grid_cols", 6
            )
            ...
            stats = {
                ...
                "utilization_rate": (
                    round((seated_students / total_desks) * 100, 1)
                    if total_desks > 0
                    else 0
                ),
                "last_modified": (
                    self.context.modified().ISO8601()
                    if hasattr(self.context, "modified")
                    else None
                ),
            }
