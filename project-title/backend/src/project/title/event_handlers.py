    ...
    try:
        ...
        try:
            ...
            if annotations is not None:
                ...
                stats = annotations.get(
                    stats_key,
                    {"total_passes": 0, "total_duration": 0, "average_duration": 0},
                )

                stats["total_passes"] += 1
                stats["total_duration"] += duration
                stats["average_duration"] = (
                    stats["total_duration"] / stats["total_passes"]
                )
        ...
        try:
            ...
            if annotations is not None:
                ...
                if hasattr(obj, "students") and obj.students:
                    ...
                    logger.info(
                        f"✅ Random picker updated with {len(obj.students)} students"
                    )
...
def fire_hall_pass_issued(hall_pass_obj, student_name=None, destination=None):
    ...
    try:
        ...
        event = HallPassIssuedEvent(
            hall_pass_obj, student_name=student_name, destination=destination
        )
        notify(event)
    except Exception as e:
