...
def get_production_security_config() -> Dict[str, Any]:
    ...
    config = {
        ...
        "rate_limits": {
            "api_random_picker": int(
                os.getenv("RATE_LIMIT_PICKER", "30")
            ),  # 30 picks per hour
            "hall_pass_create": int(
                os.getenv("RATE_LIMIT_HALL_PASS", "20")
            ),  # 20 passes per hour
            "login_attempts": int(
                os.getenv("RATE_LIMIT_LOGIN", "5")
            ),  # 5 login attempts per hour
            "api_general": int(
                os.getenv("RATE_LIMIT_GENERAL", "100")
            ),  # 100 API calls per hour
            "dashboard_refresh": int(
                os.getenv("RATE_LIMIT_DASHBOARD", "200")
            ),  # 200 dashboard updates per hour
        },
        ...
        "ferpa_compliance": {
            ...
            ),  # 7 years for educational records
        },
        ...
        "file_upload": {
            ...
            "virus_scanning_enabled": os.getenv("VIRUS_SCANNING", "false").lower()
            == "true",
        },
        # API Security
        "api_security": {
        ...
        "backup": {
            ...
            "offsite_backup_enabled": os.getenv("OFFSITE_BACKUP", "false").lower()
            == "true",
        },
    }
...
def validate_security_config(config: Dict[str, Any]) -> List[str]:
    ...
    if audit_retention < 365:
        warnings.append(
            "WARNING: Audit retention less than 1 year may not meet compliance requirements"
        )
...
def get_security_config_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        "monitoring_enabled": config.get("monitoring", {}).get(
