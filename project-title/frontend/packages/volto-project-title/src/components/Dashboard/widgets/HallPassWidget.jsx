...
import { Segment, Header, Icon, Label, List, Message, Statistic } from 'semantic-ui-react';
...
const HallPassWidget = ({ data }) => {
  ...
  return (
    <Segment className="dashboard-widget hall-pass-widget" style={{ marginBottom: '20px' }}>
      <Header as="h3">
          ...
          <List divided relaxed>
            {passes.map(pass => (
              <List.Item key={pass.id}>
                <List.Content>
                  <List.Header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span>
                      <Icon name="user" />
                      {pass.student}
                    </span>
                  ...
                  <List.Description style={{ marginTop: '5px' }}>
                    ...
                    <span style={{ marginLeft: '15px', color: '#666' }}>
                      <Icon name="clock" />
                      Since {new Date(pass.issue_time).toLocaleTimeString([], { 
                        hour: '2-digit', 
                        minute: '2-digit' 
                      })}
                    </span>
          ...
          {/* Alert Summary */}
             ...
             <Message warning style={{ marginTop: '10px' }}>
              ...
              <p>
                ...
                {' '}ha{data.alerts.length > 1 ? 've' : 's'} been out for an extended time.
              </p>
            </Message>
          )}
        </div>
