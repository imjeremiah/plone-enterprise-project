...
const ParticipationWidget = React.lazy(
  () => import('./widgets/ParticipationWidget'),
);
...
const TeacherDashboardOptimized = React.memo(() => {
    ...
    async (isRetry = false) => {
      ...
      try {
        ...
        const response = await fetch(
          `${apiUrl}/@@api-batch?endpoints=dashboard,hall_passes,seating,participation,timers`,
          {
            method: 'GET',
            credentials: 'include',
            headers: {
        ...
        const mergedData = {
          ...
          seating: data.data.seating ||
            dashboardData?.seating || { status: 'no_charts', charts: [] },
          hall_passes: data.data.hall_passes ||
            dashboardData?.hall_passes || { active_passes: [], count: 0 },
          participation: data.data.participation ||
            dashboardData?.participation || { fairness_score: 100 },
          timers: data.data.timers ||
            dashboardData?.timers || { active_timers: [], count: 0 },
          ...
          quick_stats: {
            ...
            students_picked_today:
              data.data.participation?.students_picked_today || 0,
  ...
  const startPolling = useCallback(() => {
    ...
    const hasActiveContent =
      dashboardData?.quick_stats?.active_passes > 0 ||
