...
import { Segment, Statistic, Label, Icon, Modal, Button } from 'semantic-ui-react';
...
const PerformanceMonitor = () => {
  ...
  const testEndpoint = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/Plone/@@dashboard-metrics', {
        method: 'GET',
        mode: 'cors',
        headers: {
    ...
    try {
      ...
      response = await fetch('http://localhost:8080/Plone/@@dashboard-metrics', {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        }
      ...
      try {
        ...
        console.log('❌ Dashboard fallback also failed:', fallbackError.message);
      }
    }
  ...
  const getModeIcon = (mode) => {
    switch (mode) {
      ...
      case 'pending': return { name: 'hourglass half', color: 'orange' };
      case 'waiting': return { name: 'hourglass half', color: 'blue' };
  ...
  const getIndexStatusSummary = () => {
    ...
    const available = Object.values(metrics.index_status).filter(Boolean).length;
  ...
  return (
    <>
      <Segment size="mini" className="performance-monitor" style={{ padding: '8px' }}>
        <div 
        ...
        >
             ...
             <Statistic.Value style={{ 
               color: metrics.note ? 'orange' : getPerformanceColor(metrics.query_time_ms),
               fontSize: '1rem'
             }}>
          ...
          <div className="performance-details" style={{ marginTop: '10px' }}>
            <div style={{ display: 'flex', gap: '5px', marginBottom: '8px' }}>
              ...
              <Label size="tiny">
               ...
               <Label size="tiny" color={indexSummary.available === indexSummary.total ? 'green' : 'yellow'}>
                 <Icon name="database" />
        ...
        <Modal.Content>
          <Segment>
            ...
            <Statistic.Group size="mini" widths="three">
              ...
              <Statistic>
                ...
                <Statistic.Label>Hall Pass Mode: {metrics.hall_pass_mode}</Statistic.Label>
              </Statistic>
              ...
              <Statistic>
                ...
                <Statistic.Label>Seating Mode: {metrics.seating_mode}</Statistic.Label>
              </Statistic>
            </Statistic.Group>
          </Segment>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '5px' }}>
                {Object.entries(metrics.index_status).map(([index, available]) => (
                 ...
                 <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
                   <li><Icon name="lightning" color="green" /> Optimized: Uses indexes for fast O(log n) queries</li>
                   <li><Icon name="clock" color="yellow" /> Fallback: Falls back to slower O(n) scanning</li>
                   <li><Icon name="warning" color="red" /> Error: Query failed, check system status</li>
                   <li><Icon name="hourglass half" color="orange" /> Pending: Backend needs add-on reinstall</li>
                   <li><Icon name="hourglass half" color="blue" /> Waiting: Backend registration in progress</li>
                 </ul>
               </div>
            </Segment>
          )}
          ...
          <Segment>
            ...
            <div style={{ fontSize: '0.9em' }}>
              <p><strong>Query Time Benchmarks:</strong></p>
              <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
              ...
              <p style={{ marginTop: '10px' }}>
                <strong>Last Updated:</strong> {new Date(metrics.timestamp).toLocaleTimeString()}
              </p>
            </div>
          </Segment>
        </Modal.Content>
