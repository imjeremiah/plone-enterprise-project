...
const TimerDisplay = React.memo(
  ({ remaining, duration, isRunning, compact = false }) => {
...
const LessonTimerOptimized = React.memo(
  ({ presets = [], contentUrl, compact = false }) => {
    ...
    useEffect(() => {
      ...
      AudioManager.preloadSound(
        'warning',
        '/++resource++project.title/sounds/warning.mp3',
      );
      AudioManager.preloadSound(
        'complete',
        '/++resource++project.title/sounds/complete.mp3',
      );
    }, [loadPresets]);
      ...
      () => (
        <div className="timer-controls">
          {!isRunning ? (
            ...
            <Button orange icon="pause" content="Pause" onClick={pauseTimer} />
          )}
          <Button secondary icon="stop" content="Reset" onClick={resetTimer} />
          {!compact && (
            <Button
              basic
              icon="expand"
